...
//			var $ipt = $('<input type="text"  value="" class="select_rel" />');
...
//			var $all = $('<input type="checkbox" class="select_all"/><label>'+options.selectallTxt+'</label>');//全选
			var $all = $('<input type="hidden" class="select_all"/><label>'+options.selectallTxt+'</label>');//全选
			...
			$btn.click(function (){//点击 ok按钮 
				...
				if(options.type == 1){
					 ...
					 $("#color"+options.value).val($ipt.val());
					
				}
				if(options.type == 2){
					 $("#defective"+options.value).val($ipt.val());
					
				}
				if(options.type == 3){
					 $("#remarks"+options.value).val($ipt.val());
