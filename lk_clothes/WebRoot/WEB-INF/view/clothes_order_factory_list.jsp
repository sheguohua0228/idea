...
<html>
  ...
  <body class="list">
	...
	<div class="body">
		<form id="listForm" action="clothesOrder/factory" method="post">
			<div class="listBar">
				<label>查找: </label>
				子条形码：
			...
			<table id="listTable" class="listTable">
				...
				<c:forEach var="order" items="${pager.dataList}">
					<tr>
						...
						<td>
							<a href="clothesOrder/view?id=${order.id}&type=1" title="查看">[查看]</a>
							<%-- <a href="clothesOrder/singleDirectFactoryTowash?clothesOrderId=${order.id}" title="下一步">[下一步]</a> --%>
						</td>
					</tr>
				</c:forEach>
				</c:if>
			...
			<c:if test="${fn:length(pager.dataList) > 0 }">
				<div class="pagerBar">
				<div class="delete">
					<input type="button" id="printButton"  class="formButton" url="sorting" value="打印衣物" disabled/> 
					<span title="必须先打印出分拣衣物跟踪表" style="cursor: help;">(?) </span>
