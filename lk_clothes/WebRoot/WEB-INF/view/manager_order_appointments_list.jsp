...
<html>
  ...
  <body class="list">
	...
	<div class="body">
		<form id="listForm" action="managerOrder/appointment" method="post">
			<div class="listBar">
			...
			<table id="listTable" class="listTable">
				...
				<c:forEach var="order" items="${pager.dataList}">
					<tr>
						...
						<c:if test="${order.orderStatus == 1 }">进行中</c:if>	
						<c:if test="${order.orderStatus == 2 }">待支付</c:if>	
						<c:if test="${order.orderStatus == 3 }">已完成</c:if>
						<c:if test="${order.orderStatus == 9 }">订单已取消</c:if>
						</td>
						<td>
							<fmt:formatDate value="${order.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
						</td>
						<td>
							<fmt:formatDate value="${order.backTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
						</td>
						<td>
							<a href="managerOrder/view?id=${order.id}" title="查看">[查看]</a>
							
							  <a class="invalidBtn" href="javascript:" orderId="${order.id}" title="取消">[取消]</a>
							  
	
							<a href="managerOrder/changeOrderEmployeeView?OrderId=${order.id}">[改派]</a>
						</td>
					</tr>
				</c:forEach>
				</c:if>
