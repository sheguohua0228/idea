...
<table id="listTable" class="listTable">
				...
				<c:forEach var="order" items="${pager.dataList}">
					<tr>
						...
						<td>
							<c:if test="${order.exStatus  == 0}">
								未提取
							</c:if>
							<c:if test="${order.exStatus  == 1}">
								已提取
							</c:if>
						</td>
						<td>
							<span title="${order.createDate}"><fmt:formatDate value="${order.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/> </span>
						</td>
						 
						<td>
							<a href="http://www.leyes.me/liankun/admin/print_order!view.action?id=${order.id}" title="查看">[查看]</a>
